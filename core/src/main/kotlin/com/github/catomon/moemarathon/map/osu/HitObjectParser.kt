...
fun hitObjectToNotePosition(x: Int, y: Int): Float {
    ...
    val initialPosition = 100f / PlayScreen.GameplayConfig.hitZonesAmount * (PlayScreen.GameplayConfig.hitZonesAmount * degrees).toInt() / 100f
    return initialPosition
}

fun HitObject.toNote(tracingNext: Boolean = false, tracingPrev: Boolean = false): Note { //512x384
    return Note(time / 1000f, hitObjectToNotePosition(x, y), tracingNext, tracingPrev, isNextColor = isNewCombo, colorIndex = comboIndex)
}
